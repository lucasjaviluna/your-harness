...
export class Review extends AggregateRoot<ReviewId> {
  ...
  start(): Review {
    if (this.status !== ReviewStatus.Pending) {
      throw new Error(
        "Only pending reviews can be started."
      );
    }
  ...
  approve(): Review {
    if (this.status !== ReviewStatus.InProgress) {
      throw new Error(
        "Only in-progress reviews can be approved."
      );
    }
  ...
  reject(): Review {
    if (this.status !== ReviewStatus.InProgress) {
      throw new Error(
        "Only in-progress reviews can be rejected."
      );
    }
  ...
  requestChanges(): Review {
    if (this.status !== ReviewStatus.InProgress) {
      throw new Error(
        "Only in-progress reviews can request changes."
      );
    }
